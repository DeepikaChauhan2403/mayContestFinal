...
const your_api_key = 'JBfYhp9cmEsMTXyrqoFIg6UPl41rpDuHYpYTjGAl'; // Replace with your NASA API key
...
function getImageOfTheDay(event){
    ...
    if(yourDate>date){
        alert("Invalid date selection. Please choose a date on or before: " + date);
        return;
    }
...
function addSearchToHistory(yourDate){
    let list = JSON.parse(localStorage.getItem('searches'));
    ...
    list.forEach(element => {
        ...
        link.textContent = element.date;
        link.addEventListener("click",function() {
                box_headding.innerText = `Picture on ${element.date}`
                callAPI(element.date);
            });
            li.appendChild(link);
